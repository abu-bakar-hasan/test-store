...
export default async function BrandCategoryPage({ params }: { params: Promise<{ brand: string, category: string }> }) {
  ...
  return (
    <div className="w-full min-h-screen bg-white dark:bg-slate-950 flex flex-col">
      ...
      <header className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 px-4 py-4 sticky top-0 bg-white dark:bg-slate-950 z-10">
        <Link href={`/shop/brand/${brand}`} className="p-1 hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors rounded-full text-black dark:text-white">
          <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
          </svg>
        </Link>
        <h1 className="text-sm font-bold text-black dark:text-white tracking-widest uppercase truncate ml-2 mr-2">
          {brand.replace('-', ' ')} / {category.replace('-', ' ')}
        </h1>
        <button className="p-1 hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors rounded-full text-black dark:text-white flex-shrink-0">
          <svg className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <main className="flex-1 p-4">
        <div className="grid grid-cols-2 gap-4">
          {[1, 2, 3, 4, 5, 6].map((item) => (
            <Link 
              href={`/shop/product/prod-${brand}-${category}-${item}`}
              key={item}
            ...
            >
              ...
              <div className="p-3 border-t border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950 flex flex-col">
                <h3 className="text-[10px] sm:text-xs font-medium text-black dark:text-white uppercase tracking-wide truncate mb-1">[{brand} {category} {item}]</h3>
